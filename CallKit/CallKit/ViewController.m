...
//  CallKit
//
//  Created by ss-iOS-LLY on 16/10/12.
...
- (void)viewDidLoad {
    ...
    noTextField = [[UITextField alloc] initWithFrame:CGRectMake((self.view.frame.size.width-100)/2, 50, 120, 20)];
...
-(void)checkPermissions:(UIButton *)sender
{
    ...
    // 获取权限状态
    [manager getEnabledStatusForExtensionWithIdentifier:@"com.CallDirectoryExtension" completionHandler:^(CXCallDirectoryEnabledStatus enabledStatus, NSError * _Nullable error) {
        if (!error) {
            NSString *title = nil;
...
{
    CXCallDirectoryManager *manager = [CXCallDirectoryManager sharedInstance];
    [manager reloadExtensionWithIdentifier:@"com.CallDirectoryExtension" completionHandler:^(NSError * _Nullable error) {
        if (error == nil) {
            UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"提示"
                                                                           message:@"更新成功"
                                                                    preferredStyle:UIAlertControllerStyleAlert];
